...
export const editItem = (ref, id, {description, size, quantity, category, additional_details, attachment}) => {
  return (dispatch) => {
    let data = new FormData();
    data.append('size', size);
    data.append('category', category);
    ...
    dispatch({type: FETCH_ITEM_START});
    axios.post('user/orders/'+ref+'/items/'+id, data, config)
    .then(({data}) => {
      console.log(data.item);
      if (data) {
        dispatch({type: FETCH_ITEM_SUCCESS});
        dispatch({type: NEW_ITEM, payload: data.item});
        dispatch({type: EDIT_ITEM_SUCCESS, payload: true});
      } else {
        console.log("payload: data.error", data.error);
        dispatch({type: FETCH_ITEM_ERROR, payload: "Network Error"});
      }
